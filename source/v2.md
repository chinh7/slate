...
Authentication requires 2 parameters: `User ID` and `User Secret`. These parameters can be obtained from
[Setting page](https://www.quoine.com/app/#/app/settings)

We use [JSON Web Token](https://jwt.io) open standard for Authentication. You can choose to manually authenticate by following JWT specs. But we highly recommend using a JWT library, which is available in a wide range of programming languages.  

The authencation involves signing an payload, which includes the following parameters

<h4>Auth Payload Params:</h4>

Param   | Optional? | Description
---------|-----------|------------
path || request path, e.g `/orders?currency_pair_code=BTCUSD`
nonce || an ever-increasing number, uniquely created for each request. We recommend using a millisecond timestamp, e.g `1459142524488`
